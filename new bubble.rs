...
use rayon::prelude::*; // Added for extreme CPU efficiency
use anyhow::{Result, Context}; // Added for enterprise error handling

// ==============================================================================
// 1. THE MATHEMATICAL CORE (MP4c HDC & God Node Engine) - FULLY PRESERVED
// ==============================================================================
...
impl Hypervector {
    ...
    pub fn bind(&self, other: &Self) -> Self {
        ...
        Hypervector {
            data,
            concept_id: format!("bound{}_{}", self.concept_id, other.concept_id),
...
pub fn associative_attention(query: &Hypervector, keys: &[Hypervector], values: &[Hypervector], temperature: f64) -> Hypervector {
...
impl MemoryTrace {
    pub fn new(hv: Hypervector, tier: MemoryTier, r: f64, theta: f64, phi: f64, task_context: Option<String>) -> Self {
...
impl<'a> CognitiveEngine<'a> {
    ...
    pub fn evaluate_lasso_fitness(candidate: &ASTNode, target: &Hypervector, lambda_l1: f64) -> f64 {
...
impl HDQAgent {
    ...
    pub fn learn(&mut self, state: &Hypervector, action_name: &str, reward: f64, _next: Option<&Hypervector>) {
...
// ==============================================================================
// 3. ENTERPRISE KNOWLEDGE BUBBLE (Upgraded Efficiency & Error Handling)
// ==============================================================================
...
impl UnifiedKnowledgeBubble {
    pub fn new() -> Self {
        // UPGRADE: Configured Sled for max performance with 256MB cache
        let db = sled::Config::default()
            .path("god_node_storage")
            .cache_capacity(256 * 1024 * 1024)
            .open().expect("Failed to open sled database");
    ...
    pub fn set_task(&mut self, task_name: &str) {
        let hv = self.task_vectors.entry(task_name.to_string())
    ...
    pub fn ingest(&mut self, text: &str, r: f64, theta: f64, phi: f64) -> String {
        ...
        if let Some(ref task) = self.active_task {
            if let Some(task_hv) = self.task_vectors.get(task) {
                hv = hv.bind(task_hv);
                // FIX: Take the first 20 characters safely, regardless of byte size
                let snippet: String = text.chars().take(20).collect();
                hv.concept_id = format!("{}:{}", task, snippet);
            }
        }
        let trace = MemoryTrace::new(hv.clone(), MemoryTier::Episodic, r, theta, phi, self.active_task.clone());
        self.episodic_buffer.push(trace);
        hv.concept_id.clone()
    }

    // UPGRADE: Enterprise File Loading, Error Handling (anyhow), and Parallel CPU Processing (rayon)
   pub fn ingest_book(&mut self, filepath: &str, title: &str) -> Result<usize> {
        let content = std::fs::read_to_string(filepath)
            .with_context(|| format!("Failed to read file from path: {}", filepath))?;
        ...
        let processed_traces: Vec<MemoryTrace> = paragraphs.par_iter().map(|p| {
            ...
            // FIX: Take the first 15 characters safely for the concept ID
            let snippet: String = p.chars().take(15).collect();
            hv.concept_id = format!("Book:{}:{}", title, snippet);
            
            MemoryTrace::new(hv, MemoryTier::Episodic, r, theta, phi, Some(format!("Book: {}", title)))
        }).collect();
    ...
    pub fn consolidate(&mut self) {
        ...
        for (concept_id, traces) in groups {
            ...
            let final_trace = MemoryTrace::new(final_hv.clone(), MemoryTier::Semantic, r, theta, phi, None);
            let encoded = bincode::serialize(&final_trace).unwrap();
            self.semantic_db.insert(concept_id.as_bytes(), encoded).unwrap();
            self.semantic_index.push(final_hv); // Update index
        }
        self.semantic_db.flush().unwrap();
        self.episodic_buffer.clear();
    }

    pub fn retrieve(&self, query: &str, task_filter: Option<&str>, top_k: usize) -> Vec<(String, f64)> {
        let mut query_hv = Hypervector::from_string(query, None);
        if let Some(task) = task_filter {
            let task_hv = self.task_vectors.get(task).cloned().unwrap_or_else(|| {
                Hypervector::from_string(&format!("task_{}", task), None)
            });
    ...
    pub fn cleanup_disk(&mut self, threshold: f64) -> usize {
        ...
        for (i, hv) in self.semantic_index.iter().enumerate() {
...
// ==============================================================================
// 4. UI STATE & 3D ENGINE - FULLY PRESERVED
// ==============================================================================
struct GodNodeUI {
...
impl Default for GodNodeUI {
    fn default() -> Self {
        ...
        Self {
            ...
            status_message: "System Online. Full Cognitive Suite Loaded.".to_string(),
...
// ==============================================================================
// 5. THE DESKTOP WINDOW & DASHBOARD - UI PRESERVED, FEATURES INJECTED
// ==============================================================================
impl eframe::App for GodNodeUI {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            for item in self.bubble.semantic_db.iter() {
                if let Ok((k, v)) = item {
                    if let Ok(trace) = bincode::deserialize::<MemoryTrace>(&v) {
                        ...
                        self.cached_3d_nodes.push((cid, trace.x as f32, trace.y as f32, trace.z as f32, trace.hv.strength));
                    }
                }
            }
        ...
        egui::SidePanel::left("control_panel").exact_width(330.0).show(ctx, |ui| {
            ui.heading(egui::RichText::new("God Node Enterprise").size(22.0).strong().color(egui::Color32::from_rgb(0, 255, 255)));
            ui.separator();
            egui::ScrollArea::vertical().show(ui, |ui| {
                ui.add_space(10.0);
                ui.label(egui::RichText::new("1. Set Domain Context").strong());
                let active = self.bubble.active_task.as_deref().unwrap_or("None");
                ui.label(egui::RichText::new(format!("Current Active Task: {}", active)).color(egui::Color32::YELLOW));
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Set").clicked() && !self.task_text.is_empty() {
                        ...
                        self.status_message = format!("Active Task Set: {}", self.task_text);
                    }
                });

                ui.add_space(15.0);
                ui.label(egui::RichText::new("2. Manual Short-Term Ingest").strong());
                ...
                ui.horizontal(|ui| {
                    if ui.button("➕ Map to RAM").clicked() && !self.ingest_text.is_empty() {
                        ...
                        self.status_message = format!("Added to RAM: {}", cid);
                        self.ingest_text.clear();
                    }
                    if ui.button("🌙 Consolidate to SSD").clicked() {
                        let count = self.bubble.episodic_buffer.len();
                        self.bubble.consolidate();
                        self.needs_redraw = true;
                        self.status_message = format!("Saved {} traces to SSD.", count);
                    }
                });

                ui.add_space(15.0);
                ui.label(egui::RichText::new("3. Automated Document Loader").strong().color(egui::Color32::from_rgb(255, 165, 0)));
                
                // UPGRADE: Native File Dialog logic inserted here
                ui.horizontal(|ui| {
                    ui.label("File:");
                    ...
                    if ui.button("📂 Browse").clicked() {
                        if let Some(path) = rfd::FileDialog::new().add_filter("Text Files", &["txt"]).pick_file() {
                            ...
                            if self.book_title.is_empty() {
                                self.book_title = path.file_stem().unwrap_or_default().to_string_lossy().to_string();
                            }
                        }
                    }
                });
                ...
                // UPGRADE: Error handling linked to the UI log
                if ui.button("📚 Ingest Entire Book").clicked() && !self.book_path.is_empty() {
                    self.status_message = "Parsing neuro-symbolic vectors in parallel...".to_string();
                    match self.bubble.ingest_book(&self.book_path, &self.book_title) {
                        Ok(count) => {
                            self.status_message = format!("Success! {} paragraphs processed across multiple CPU cores.", count);
                            self.needs_redraw = true;
                        },
                        Err(e) => {
                            self.status_message = format!("Error: {:?}", e); // Beautiful robust error view
                        }
                    }
                }

                ui.add_space(15.0);
                ui.label(egui::RichText::new("4. SSD Semantic Search (Top-5)").strong());
                ui.text_edit_singleline(&mut self.search_text);
                ui.add_space(5.0);
                if ui.button("🔍 Calculate Resonance").clicked() && !self.search_text.is_empty() {
                    let task_filter = self.bubble.active_task.as_deref();
                    let results = self.bubble.retrieve(&self.search_text, task_filter, 5);
                    if results.is_empty() {
                        self.search_result = "No resonance found.".to_string();
                        self.active_search_id = None;
                    } else {
                        let best = &results[0];
                        self.active_search_id = Some(best.0.clone());
                        self.search_result = results.iter()
                            .map(|(cid, sim)| format!("{}: {:.2}%", cid, sim * 100.0))
                            .collect::<Vec<_>>()
                            .join("\n");
                        self.status_message = "Search complete.".to_string();
                    }
                }

                ui.add_space(15.0);
                ui.label(egui::RichText::new("5. SSD Maintenance").strong());
                if ui.button("🧹 Run Disk Garbage Collection").clicked() {
                    let swept = self.bubble.cleanup_disk(0.4);
                    self.status_message = format!("Disk Swept. Removed {} weak concepts.", swept);
                    self.needs_redraw = true;
                }

                ui.add_space(15.0);
                ui.label(egui::RichText::new("6. Cognitive Modules (AST/RL/SPO)").strong().color(egui::Color32::from_rgb(200, 100, 255)));
                if ui.button("🧠 Run Diagnostics & Agent Learning").clicked() {
                    ...
                    let ast_score = CognitiveEngine::evaluate_lasso_fitness(&elegant, &target, 0.02);
                    ...
                    self.status_message = format!("Agent chose: '{}'. AST Score: {:.4}. SPO Fact saved.", action, ast_score);
                    self.needs_redraw = true;
                }
                ...
                ui.label(egui::RichText::new("System Log:").color(egui::Color32::GRAY));
                ui.label(egui::RichText::new(&self.status_message).color(egui::Color32::WHITE));
                ui.add_space(10.0);
                if !self.search_result.is_empty() {
                    ui.label(egui::RichText::new(&self.search_result).color(egui::Color32::GREEN).size(16.0));
                }
            });
        });

        egui::CentralPanel::default().show(ctx, |ui| {
            ui.heading("3D Sled-Backed Knowledge Sphere");
            ...
            let scale = 22.0;
            painter.circle_stroke(center, 16.0 * scale, egui::Stroke::new(1.0, egui::Color32::from_white_alpha(10)));

            let mut projected_nodes = Vec::new();
            for (cid, x, y, z, strength) in &self.cached_3d_nodes {
                let x1 = x * self.rot_y.cos() - z * self.rot_y.sin();
                let z1 = x * self.rot_y.sin() + z * self.rot_y.cos();
                let y2 = y * self.rot_x.cos() - z1 * self.rot_x.sin();
                let z2 = y * self.rot_x.sin() + z1 * self.rot_x.cos();
                let screen_x = center.x + x1 * scale;
                let screen_y = center.y - y2 * scale;
                projected_nodes.push((cid.clone(), screen_x, screen_y, z2, *strength));
            }
            projected_nodes.sort_by(|a, b| a.3.partial_cmp(&b.3).unwrap());

            for (cid, px, py, pz, strength) in projected_nodes {
                let pos = egui::pos2(px, py);
                let depth_factor = (pz + 15.0) / 30.0;
                let radius = 2.0 + (strength as f32 * 2.0) * depth_factor;
                let alpha = (50.0 + (205.0 * depth_factor)).clamp(0.0, 255.0) as u8;
                let (color, text_color) = if Some(&cid) == self.active_search_id.as_ref() {
                    (egui::Color32::from_rgba_unmultiplied(255, 165, 0, 255), egui::Color32::from_rgb(255, 165, 0))
                } else {
                    (egui::Color32::from_rgba_unmultiplied(0, 200, 255, alpha), egui::Color32::from_white_alpha(alpha))
                };
                painter.circle_filled(pos, radius, color);
                if depth_factor > 0.8 || Some(&cid) == self.active_search_id.as_ref() {
                    painter.text(pos + egui::vec2(radius + 2.0, -radius), egui::Align2::LEFT_BOTTOM, &cid, egui::FontId::proportional(10.0), text_color);
                }
            }
        });
    }
...
// ==============================================================================
// 6. BOOT SEQUENCE - FULLY PRESERVED
// ==============================================================================
fn main() -> eframe::Result<()> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size([1150.0, 750.0])
            .with_title("God Node Enterprise Operating System"),
        ..Default::default()
    };

    eframe::run_native(
        "God Node Operating System",
        options,
